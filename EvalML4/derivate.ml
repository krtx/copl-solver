...
and infer_nil env = (env, Nil, VNil, ENil, [])
                      
and infer_cons env (e1, e2) =
  let (_, _, v1, _, _) as d1 = deriv env e1 in
  let (_, _, v2, _, _) as d2 = deriv env e2 in
  (env, Cons (e1, e2), VCons (v1, v2), ECons, [Dd d1; Dd d2])

and infer_match env (e1, e2, x, y, e3) =
  let (_, _, v, _, _) as d1 = deriv env e1 in
  match v with
    VNil -> let (_, _, v, _, _) as d2 = deriv env e2 in
            (env, Match (e1, e2, x, y, e3), v, EMatchNil, [Dd d1; Dd d2])
  | VCons (v1, v2) ->
     let (_, _, v, _, _) as d2 = deriv ((y, v2) :: (x, v1) :: env) e3 in
     (env, Match (e1, e2, x, y, e3), v, EMatchCons, [Dd d1; Dd d2])
  | _ -> failwith "match: type"
